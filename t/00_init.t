...
sub check_pw {
    ...
    is(DBI->connect(
       ...
       ->selectall_arrayref(
        'SELECT COUNT(u.rowid) FROM users u INNER JOIN users_config c ON c.users_id=u.rowid WHERE u.name=? AND c.password=? AND u.is_active=1'
...
sub test_path {
    ...
    my $cookie  = Testinit::test_randstring(4);
    note 'test error without short cookie name';
    my $out0 = qx($script -d $cookie 2>&1);
    like $out0,
        qr~error: cookie name provided by "-d" parameter needs to be at least 16 characters~,
        'error message for short cookie name ok';
    check_paths($testpath, 1);

    my $cookie  = Testinit::test_randstring(6);
    ...
    my $out2 = qx($script -d $cookie 2>&1);
    ...
    $cookie  = Testinit::test_randstring(6);
    my $out3 = qx(FFC_DATA_PATH=$testpath $script -d $cookie 2>&1);

    like $out3, $_, 'first run content ok' for (
    ...
    like $out4, $_, 'second run content existing ok' for (
        qr~ok: using '\d+' as data path owner and '\d+' as data path group~,
        qr~ok: using '$testpath/uploads' as upload store~,
    ...
    unlike $out4, qr($_), "second run content ok"
    ...
    $cookie  = Testinit::test_randstring(6);
    my $out5 = qx(FFC_DATA_PATH=$testpath $script -d $cookie 2>&1);
    like $out5, $_, 'third run content without database' for (
        qr~ok: using '\d+' as data path owner and '\d+' as data path group~,
        qr~ok: using '$testpath/uploads' as upload store~,
        qr~ok: path '$testpath/uploads' as upload allready exists~,
        qr~ok: using '$testpath/database\.sqlite3' as database store~,
