...
{
    note('sub update_user_stats( $userid, $act, $category ); # call_check');
    check_call(
        \&Ffc::Data::Board::update_user_stats,
        update_user_stats => {
            name => 'userid',
            good => Ffc::Data::Auth::get_userid(
                Test::General::test_get_rand_user()->{name}
            ),
            bad => [
                '',                      '        ',
...
{
    note('sub update_user_stats( $userid, "msgs" ); # data test msgs');
    ...
    ok(
        (
            just_call(
                sub { Ffc::Data::Board::update_user_stats( $userid, 'msgs' ) }
            )
        )[0],
        'test run ok'
    );
...
{
    note(
'sub update_user_stats( $userid, "forum" ); # data test forum without category'
    );
    my $user         = Test::General::test_get_rand_user();
    my $userid       = Ffc::Data::Auth::get_userid( $user->{name} );
    my $dbh          = Ffc::Data::dbh();
    ...
    ok(
        (
            just_call(
                sub {
                    Ffc::Data::Board::update_user_stats( $userid, 'forum' );
                }
            )
        )[0],
        'test run ok'
...
{
    note(
'sub update_user_stats( $userid, "forum", $category ); # data test forum with category'
    );
    my $user         = Test::General::test_get_rand_user();
    my $userid       = Ffc::Data::Auth::get_userid( $user->{name} );
    ...
    cmp_ok($after_first_cat, 'lt', $after_second_cat, 'statistics updated ok in lastseenforum');
}
