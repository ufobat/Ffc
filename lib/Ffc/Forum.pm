...
sub install_routes { 
    my $l = shift;
    $l->route('/forum/query')->via('post')
      ->to(controller => 'forum', action => 'query_userlist')
      ->name('query_forum_userlist');
    $l->route('/forum')->via('get')
      ->to(controller => 'forum', action => 'show_userlist')
      ->name('show_forum_topiclist');
    Ffc::Plugin::Posts::install_routes_posts($l, 'forum', '/forum/:topicid', topicid => $Ffc::Digqr);
}
...
sub show_userlist {
    ...
    $c->stash( topics => $c->dbh->selectall_arrayref(
        'SELECT u."id", u."name",
            (SELECT COUNT(p."id") 
                FROM "posts" p
