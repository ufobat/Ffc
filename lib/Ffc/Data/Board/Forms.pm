...
sub delete_post {
    ...
    $from = get_userid( $from, 'Auto des zu löschenden Beitrages' );
    my $dbh = Ffc::Data::dbh();
...
sub insert_post {
    my ( $f, $d, $c, $t ) = @_;
    $f = get_userid( $f, 'Autor des neuen Beitrages' );
    $t = get_userid( $t, 'Empfänger des neuen Beitrages' ) if $t and $t != $f;
...
sub update_post {
    my ( $f, $d, $i ) = @_;
    $f = get_userid( $f, 'Autor des bestehenden Beitrages' );
    die qq{Beitrag ungültig, zu wenig Zeichen (min. 2)} if 2 >= length $d;
