...
sub check_username { &Ffc::Data::Auth::check_username }
...
sub admin_update_password {
    my $adminuid = get_userid(shift, 'Administrator für Passwortänderung');
    die 'Passworte von anderen Benutzern dürfen nur Administratoren ändern'
        unless Ffc::Data::Auth::is_user_admin($adminuid);
    my $userid = get_userid(shift, 'zu bearbeitender Benutzer für Passwortänderung');
    my $pw1 = shift;
    my $pw2 = shift;
...
sub admin_update_active {
    my $adminuid = get_userid(shift, 'Administrator für Aktivierung/Deaktivierung');
    die 'Benutzern aktivieren oder deaktiveren dürfen nur Administratoren'
        unless Ffc::Data::Auth::is_user_admin($adminuid);
    my $userid = get_userid(shift, 'zu bearbeitender Benutzer für Aktivierung/Deaktivierung');
    my $active = shift() ? 1 : 0;
...
sub admin_update_admin {
    my $adminuid = get_userid(shift, 'Administrator für Administratoreneinstellung');
    die 'Benutzern zu Administratoren befördern oder ihnen den Adminstratorenstatus wegnehmen dürfen nur Administratoren'
        unless Ffc::Data::Auth::is_user_admin($adminuid);
    my $userid = get_userid(shift, 'zu bearbeitender Benutzer für Administratoreneinstellung');
    my $admin = shift() ? 1 : 0;
...
sub admin_create_user {
    my $adminuid = get_userid(shift, 'Administrator zum Anlegen eines neuen Benutzers');
    die 'Neue Benutzer anlegen dürfen nur Administratoren'
        unless Ffc::Data::Auth::is_user_admin($adminuid);
    my $username = shift;
    check_username($username) or die qq(Benutzer "$username" existiert bereits und darf nicht neu angelegt werden);
    my $pw1 = shift;
    my $pw2 = shift;
    _check_password_change( $pw1, $pw2 );
    my $active = shift() ? 1 : 0;
